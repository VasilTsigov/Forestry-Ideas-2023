...
class WA_MySQLi_RS  {
  ...
  public $TotalRows;
	...
	public function __construct($name,$conn,$maxRows=0,$skipRows=0) {
	  ...
	  $this->CurrentPage = (isset($_SERVER['REQUEST_URI'])?$_SERVER['REQUEST_URI']:"");
      ...
      $this->Columns = mysqli_fetch_fields($query);
        ...
        while ($rows = $result->fetch_array(MYSQLI_ASSOC)) {
          $this->addRow($rows);
            ...
            $this->Results = $this->waMySQLiResultFetchAssoc($result);
		  }
	  }
	...
	public function filterResults($filters = false) {
		...
		for ($x=0; $x<sizeof($this->Results); $x++) {
		  $returnResult = true;
		  for ($y=0; $y<sizeof($filters) && $returnResult == true; $y++) {
            if (is_array($filters[$y][1])) {
              ...
              if (isset($filters[$y][2])) {
                if ($filters[$y][2] == ">=") {

                  if (floatval($this->Results[$x][$filters[$y][0]]) < floatval($filters[$y][1])) {
                    $returnResult = false;
                  } 
                } else if ($filters[$y][2] == "<=") {
                  if (floatval($this->Results[$x][$filters[$y][0]]) > floatval($filters[$y][1])) {
              ...
              } else if (strtolower($this->Results[$x][$filters[$y][0]]) != strtolower($filters[$y][1])) {
                $returnResult = false;
    ...
    public function getDecryptedVal($col, $salt="") {
      ...
      if ($colVal) {
        $colVal = $this->getDecryptedStr($colVal, $salt);
      }
      return $colVal;
    }

    public function getDecryptedStr($colVal, $salt="") {
	...
	public function getPageLink($page) {
      $startPage = $this->getPageURL();
	  return $startPage .= "&" . $this->PageParam . "=" . $page;
	}
	...
	public function getPageURL() {
	  ...
	  return $startPage . "?totalRows_" . $this->Name . "=" . $this->TotalRows . (sizeof($urlParams)>0?"&".implode("&",$urlParams):"");
	}
  ...
  public function setPagination() {
	...
	private function showError($errorMessage) {
      if ($this->Debug) {
        ...
        die($errorMessage."<br><br>".str_replace("\n","<br>",substr($trace,strpos($trace,"#1"),strrpos($trace,"#")-strpos($trace,"#1"))));
      } else {
	...
	private function waMySQLiResultFetchAssoc(&$result) {
	  $ret = array();
      $retRows = array();
      mysqli_stmt_store_result($result->stmt);
	  ...
	  while (mysqli_stmt_fetch($result->stmt)) { 
          $retRows[] = unserialize(serialize($ret));
      }
	  return $retRows;
	}
